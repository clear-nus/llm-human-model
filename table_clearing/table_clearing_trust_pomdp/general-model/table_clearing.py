...
class TableFileGenerator:
    ...
    def __init__(self, numBottle, numCan, numGlass, discount):
 ...
 (1,
  'Water Bottle'): array([[  1.84075573e-01,   7.22940628e-01,   9.25969539e-02,
    ...
    def StateGen(self):
        ...
        for i in range(self.numObjStates-1):
            objStatus = self.Baseb(i, self.numStatus)
            objState = ''
            for j in range(self.numObjects):
                objState += "ObjStatus" + str(j) + objStatus[j]

            objectStates.append(objState)
            ...
            for trustState in self.trustStates:
                if objState != "END":
                    for action in self.actions:
                        objStatus = self.toObjState(objState)
                        # print objStatus
    ...
    def newObjStates(self, objStatus, tr, act, objState):
        t = []
        if objStatus[act] != 0:
            # newObjState = self.toObjString(objStatus, -1, -1)
            self.addStatetoTrans(t, objState, 1.0)
        else:
            humanActsProb = self.HumanActionsProb(tr, act)

            ## intervene
            newObjState = self.toObjString(objStatus, act, 0)
            self.addStatetoTrans(t, newObjState, humanActsProb[0])

            ## stay put
            newObjState = self.toObjString(objStatus, act, 1)
    ...
    def toObjState(self, ss):
        state = []
        for i in range(self.numObjects):
            pattern1 = 'tus' + str(i)
            pattern2 = 'ObjStatus' + str(i+1)
            start = ss.find(pattern1)
            if i < self.numObjects - 1:
                end = ss.find(pattern2)
            else:
                end = len(ss)
            s = int(ss[start+4:end])
            state.append(s)
        return state


    def toObjString(self, objStatus, act, hact):
        if hact == 1:
            # stay put
           objStatus[act] = 1
        elif hact == 0:
        ...
        if endState(objStatus):
            ...
            nstate = ''
            for i in range(self.numObjects):
                nstate += 'ObjStatus' + str(i) + str(objStatus[i])
            return nstate
...
if __name__=='__main__':
    if len(sys.argv) != 5:
        print 'Usage: python table_clearing.py num-bottle num-can num-glass discount-factor'
        sys.exit(0)
    ...
    xmlTable = TableFileGenerator(numBottle, numCan, numGlass, discount)
    filename = str(numBottle) + 'bottle-' + str(numCan) + \
            'can-' + str(numGlass) + 'glass-' + str(discount) + '.pomdpx'
    xmlTable.XmlGen("pomdpx/" + filename)
