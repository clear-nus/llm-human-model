...
class TableFileGenerator:
    ...
    def __init__(self, discount):
        self.discount = float(discount)        #reward discount value
    ...
    def InitTrustDyna(self):
        ...
        # Init trust dynamics
        with open('trust_models/trust_dynamics_no_transfer.json', 'r') as f:
        ...
        for i in range(self.numTrustLevels):
            bottlei = min(self.numTrustLevels - 1, i + dyna_table['Water Bottle'])
            tmp = [0] * self.numTrustLevels
            tmp[bottlei] = 1.0
            trustDyna[1, 'Water Bottle'].append(copy(tmp))
            glassi = min(self.numTrustLevels - 1, i + dyna_table['Glass Cup'])
            tmp = [0] * self.numTrustLevels
            tmp[glassi] = 1.0
...
xmlTable.XmlGen("pomdpx/tableClearingNoTransferd" + sys.argv[1][2:] + ".pomdpx")
