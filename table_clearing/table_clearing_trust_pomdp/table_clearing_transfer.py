...
class TableFileGenerator:
    ...
    def __init__(self, discount):
        self.discount = float(discount)        #discount 
    ...
    def InitTrustDyna(self):
        ...
        # Init trust dynamics
        with open('trust_models/trust_dynamics_transfer.json', 'r') as f:
        ...
        for ptask in list_tasks:
            ...
            for i in range(self.numTrustLevels):
                bottlei = min(self.numTrustLevels - 1, i + dyna_table[ptask]['Water Bottle'])
                tmp = [0] * self.numTrustLevels
                tmp[bottlei] = 1.0
                trustDyna[(1, 'Water Bottle')][ptask].append(copy(tmp))
                glassi = min(self.numTrustLevels - 1, i + dyna_table[ptask]['Glass Cup'])
                tmp = [0] * self.numTrustLevels
                tmp[glassi] = 1.0
            ...
            for i1 in range(self.numStatus):
                ...
                for i2 in range(self.numStatus):
                    ...
                    for i3 in range(self.numStatus):
                        ...
                        objectStates.append(objectState3)
        objectStates.append("END")
        
        for w in range(self.numTrustLevels):
            for k in range(self.numTrustLevels):
                trustState = "Water" + str(w) + "Glass" + str(k)
    ...
    def newTrustStates(self, objStatus, tr, act, trustState):
        ...
        if objStatus[act] != 0:
            ...
            if act < 3:
                # performance is on water bottle
                # print self.trustDyna[(0,'Water Bottle')].keys()
                # print tr
                newTrustDistri_intervene_water = self.trustDyna[(0,'Water Bottle')][tr[0]]
                newTrustDistri_intervene_glass = self.trustDyna[(0,'Glass Cup')][tr[1]]
            else:
                # performance is on glass cup 
                newTrustDistri_intervene_water = self.trustDyna[(0,'Water Bottle')][tr[0]]
                newTrustDistri_intervene_glass = self.trustDyna[(0,'Glass Cup')][tr[1]]
            ...
            if act < 3:
                # performance is on water bottle
                newTrustDistri_stay_water = self.trustDyna[(1,'Water Bottle')]['Water Bottle'][tr[0]]
                newTrustDistri_stay_glass = self.trustDyna[(1,'Glass Cup')]['Water Bottle'][tr[0]]
            else:
                # performance is on glass cup 
                newTrustDistri_stay_water = self.trustDyna[(1,'Water Bottle')]['Glass Cup'][tr[1]]
                newTrustDistri_stay_glass = self.trustDyna[(1,'Glass Cup')]['Glass Cup'][tr[1]]

            for i in range(len(newTrustDistri_intervene_water)):
                for j in range(len(newTrustDistri_intervene_glass)):
                    prob_trust_i_water = HumanActionsProb[0] * newTrustDistri_intervene_water[i] + HumanActionsProb[1] * newTrustDistri_stay_water[i];
                    prob_trust_j_glass = HumanActionsProb[0] * newTrustDistri_intervene_glass[j] + HumanActionsProb[1] * newTrustDistri_stay_glass[j];
                    newTrustState = toTrustString(i, j)
                    self.addStatetoTrans(t, newTrustState, prob_trust_i_water * prob_trust_j_glass)
        return t
    ...
    def HumanActionsProb(self, tr, act):
        ...
        if act < 3:
            # water bottle
            dictionary[0] = self.humanBehav['Water Bottle'][tr[0]][0]
        elif act == 3:
            dictionary[0] = self.humanBehav['Glass Cup'][tr[1]][0]
...
def toObjString(objStatus, act, hact):
    ...
    if endState(objStatus):
        ...
        return "ObjStatus0" + str(objStatus[0]) + "ObjStatus1" + str(objStatus[1]) + "ObjStatus2" + str(objStatus[2]) + "ObjStatus3" + str(objStatus[3])

def toTrustString(twater, tglass):
    return "Water" + str(twater) + "Glass" + str(tglass)
...
def toTrustState(ss):
    ...
    w = int(ss[start+3:end])
    start = ss.find('ass')
    end = len(ss)
    g = int(ss[start+3:end])
    return (w, g)

def toAction(ss):
    return int(ss)

xmlTable = TableFileGenerator(sys.argv[1])
xmlTable.XmlGen("pomdpx/tableClearingTransferd" + sys.argv[1][2:] + ".pomdpx")
